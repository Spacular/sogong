...
int CManageToolLogin::isExist(CString ID, CString Pwd) {
	...
	if ((retcode = SQLPrepare(hstmt1, pwd_query/*보낼 SQL 문자열*/, SQL_NTS)) != SQL_SUCCESS)
		AfxMessageBox(Errmsg);
	else if ((retcode = SQLBindParameter(hstmt1, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, sizeof(pwd), 0, pwd, 0, &ret)) != SQL_SUCCESS)
		AfxMessageBox(Errmsg);	
	...
	if ((retcode = SQLPrepare(hstmt1, id_query/*보낼 SQL 문자열*/, SQL_NTS)) != SQL_SUCCESS)
		AfxMessageBox(Errmsg);
	else if ((retcode = SQLBindParameter(hstmt1, 1, SQL_PARAM_INPUT, SQL_C_ULONG, SQL_INTEGER, 0, 0, &id, 0, NULL)) != SQL_SUCCESS)
		AfxMessageBox(Errmsg);
	...
	if		((isExistID == TRUE) && (isExistPwd == TRUE) && (isAdmin == TRUE)) 	return TRUE;	// 일치하는 계정 찾음!
	else if ((isExistID == TRUE) && (isExistPwd == TRUE) && (isAdmin != TRUE)) 	return 0;		// 관리자 계정이 아님!
	else 																		return -1;		// 계정 자체가 없음!
}
